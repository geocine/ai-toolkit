...
export const migrateJobConfig = (jobConfig: JobConfig): JobConfig => {
  ...
  if (
    ...
    delete (jobConfig.config.process[0].sample as any).prompts;
  }
  ...
  if ('auto_memory' in jobConfig.config.process[0].model) {
    jobConfig.config.process[0].model.layer_offloading = ((jobConfig.config.process[0].model as any).auto_memory ||
      false) as boolean;
    delete (jobConfig.config.process[0].model as any).auto_memory;
  }
  return jobConfig;
};
