...
import { defaultDatasetConfig, defaultJobConfig, migrateJobConfig } from './jobConfig';
...
export default function JobNewPage() {
  ...
  useEffect(() => {
    ...
    const datasetOptions = datasets.map(name => ({ value: `${settings.DATASETS_FOLDER}/${name}`, label: name }));
    ...
    <>
      <TopBar>
        <div className="hidden md:flex items-center h-full">
          <Button className="text-gray-500 dark:text-gray-300 px-2 flex items-center h-full" onClick={() => navigate(-1)}>
            <FaChevronLeft />
          </Button>
        </div>
