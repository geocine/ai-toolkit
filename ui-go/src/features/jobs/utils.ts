...
export const handleModelArchChange = (
  ...
  if (!newArch?.additionalSections?.includes('model.layer_offloading')) {
    if ('layer_offloading' in jobConfig.config.process[0].model) {
      ...
      delete (newModel as any).layer_offloading;
      delete (newModel as any).layer_offloading_text_encoder_percent;
      delete (newModel as any).layer_offloading_transformer_percent;
  ...
  const samples = jobConfig.config.process[0].sample.samples.map(sample => {
    ...
    if (!hasSampleCtrlImg) {
      delete (newSample as any).ctrl_img; // remove ctrl_img if not applicable
    }
    return newSample;
  });
